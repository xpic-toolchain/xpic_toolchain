...
// accept '[' at name start
#define LEX_BR           (LEX_NAME | LEX_BEGIN_NAME)
// byt don't ']' at name end
#define LEX_BRE          0 

#define LEX_HASH         (LEX_NAME | LEX_BEGIN_NAME)

#define LEX_UNDERLINE    0
// '+' and '-' can be in name
//#define LEX_MINUS        (LEX_NAME | LEX_BEGIN_NAME)
//#define LEX_PLUS         (LEX_BEGIN_NAME)

#define LEX_DOLLAR 	(LEX_NAME | LEX_BEGIN_NAME)
...
//unsigned int getEXPRConstant(char *str);
unsigned int getEXPRConstant(char *str,char ConstantType);
int XPIC_getConst(char **str);
int getConstant(char *acYConstStr, char ConstantType);
//int getConstant(char *acYConstStr);
...
#define TARGET_FORMAT "elf32-xpic"

/* This macro is the BFD architecture to pass to `bfd_set_arch_mach'.  */
#define TARGET_ARCH bfd_arch_xpic
...
#define TC_PARSE_CONS_EXPRESSION(EXPR,N) xpic_parse_cons_expression (EXPR, N)
extern void xpic_parse_cons_expression (expressionS *, int);

/* You may define this macro to generate a fixup for a data
   allocation pseudo-op.  */
#define TC_CONS_FIX_NEW(FRAG,WHERE,N,EXP) xpic_cons_fix_new (FRAG, WHERE, N, EXP)
extern void xpic_cons_fix_new (fragS *,int, int, expressionS *);
...
#define TC_VALIDATE_FIX(FIXP,SEG,SKIP)                       \
  if (   (FIXP->fx_r_type == BFD_RELOC_AVR_7_PCREL           \
       || FIXP->fx_r_type == BFD_RELOC_XPIC_24_PCREL         \
       || FIXP->fx_r_type == BFD_RELOC_XPIC_LONG_JMP         \
       || FIXP->fx_r_type == BFD_RELOC_XPIC_Y17_PCREL        \
