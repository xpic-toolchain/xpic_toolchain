...
{
  int n, d;
  const unsigned N = sizeof ( fabs_vec ) / sizeof ( fabs_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = fabs_vec[i][6];
    param.l_value.low = fabs_vec[i][7];
    ret.d_value = fabs(param.d_value);
    result.l_value.high = fabs_vec[i][4];
    result.l_value.low = fabs_vec[i][5];
    if (fabs_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(fabs_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(fabs_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (fabs_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(fabs_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(fabs_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
