...
{
  int n, d;
  const unsigned N = sizeof ( fmodf_vec ) / sizeof ( fmodf_vec[0] );
  unsigned i;
  ...
  for ( i = 0; i < N; ++i )
  {
    param1.l_value.high = fmodf_vec[i][6];
    param1.l_value.low = fmodf_vec[i][7];
    param2.l_value.high = fmodf_vec[i][8];
    param2.l_value.low = fmodf_vec[i][9];
    result.l_value.high = fmodf_vec[i][4];
    result.l_value.low = fmodf_vec[i][5];

    f_param1 = (float) param1.d_value;
    f_param2 = (float) param2.d_value;
    f_result = (float) result.d_value;

    f_ret = fmodf(f_param1,f_param2);
    if (fmodf_vec[i][1] == 13)
    {
      if ( ( f_ret <= f_result+(fmodf_vec[i][2]*EPSILON) && f_ret >= f_result-(fmodf_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (fmodf_vec[i][1] == 0)
    {
      if ( !( f_ret <= f_result+(fmodf_vec[i][2]*EPSILON) && f_ret >= f_result-(fmodf_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
