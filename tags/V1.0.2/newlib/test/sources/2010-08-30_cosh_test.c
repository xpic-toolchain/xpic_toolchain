...
{
  int n, d;
  const unsigned N = sizeof ( cosh_vec ) / sizeof ( cosh_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = cosh_vec[i][6];
    param.l_value.low = cosh_vec[i][7];
    ret.d_value = cosh(param.d_value);
    result.l_value.high = cosh_vec[i][4];
    result.l_value.low = cosh_vec[i][5];
    if (cosh_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(cosh_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(cosh_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (cosh_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(cosh_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(cosh_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
