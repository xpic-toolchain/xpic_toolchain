...
#define XPIC_INT_DRAM_DEFAULT 0x1018e000
unsigned int XPIC_INT_DRAM = XPIC_INT_DRAM_DEFAULT;
  ...
  /* Parse arguments and options.  */
  {
    ...
    /* This is a synonym for "--annotate=1".  --annotate is now preferred,
     ...
     {"l", required_argument, 0, 'l'},
      ...
      {"xpic-int-dram-addr", required_argument, 0, OPT_XPIC_SET_INTDRAM},
      {0, no_argument, 0, 0}
    };
	...
	switch (c)
	  ...
	  case 0:
	    ...
	    break;
	  case OPT_XPIC_SET_INTDRAM:
	  {
		int res;
		res =  sscanf (optarg,"0x%x",&XPIC_INT_DRAM);  /* hex */
		if(res <= 0)
			res =  sscanf (optarg,"%d",&XPIC_INT_DRAM);/* dec */

		/* error, set default value */
		if(XPIC_INT_DRAM == 0)
		{
			printf("Warning: wrong argument given for option \"--xpic-int-dram-addr\". Will set to default value 0x%x\n",XPIC_INT_DRAM_DEFAULT);
			XPIC_INT_DRAM = XPIC_INT_DRAM_DEFAULT;
		}
		if( (XPIC_INT_DRAM & 0x3) != 0)
		{
			printf("Warning: argument given for option \"--xpic-int-dram-addr\" not 4-byte aligned. Will set to default value 0x%x\n",XPIC_INT_DRAM_DEFAULT);
			XPIC_INT_DRAM = XPIC_INT_DRAM_DEFAULT;
		}
...
fputs_unfiltered (_("\n\
  --xpic-int-dram-addr\n\
                     Define start-address of internal DATA RAM for xPIC target.\n\n\
"), stream);
