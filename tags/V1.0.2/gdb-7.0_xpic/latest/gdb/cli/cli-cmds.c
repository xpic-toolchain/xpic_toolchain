...

extern unsigned int XPIC_INT_DRAM;
static void xpic_int_dram_addr (char *args, int from_tty)
{
	int error=1;
	if(args!=0)
	{
		...
		if(*loc_args == '=')
			loc_args++;
		/* accept only hex numbers */
		if(loc_args[0] == '0' && loc_args[1] == 'x')
		{
			loc_args+=2;
			if(8 == strlen(loc_args))
			{
				...
				for(i=7;i>=0;i--)
				{
				    if(loc_args[i]>='0' && loc_args[i]<='9')
				    {	n = loc_args[i]-'0';  }
				    else
				    if(loc_args[i]>='a' && loc_args[i]<='f')
				    {	n = loc_args[i]-'a'+10;  }
				    else
				    if(loc_args[i]>='A' && loc_args[i]<='F')
				    {	n = loc_args[i]-'A'+10;  }
				    ...
				    {   error=1;
				    	break; /* wrong symbol found */
				    }
				    num |= n<<((7-i)*4);
				}
				if(error==0) /* set offset if no error */
				{
				   XPIC_INT_DRAM=num;
				}
			}
	...
	if(error)
		printf_unfiltered(_("wrong format, usage: xpic-int-dram-addr=0x1018e000\n"));
}
...
init_cli_cmds (void)
{
  ...
  add_cmd ("xpic-int-dram-addr", no_class, xpic_int_dram_addr,_("Defines the start of INT_DRAM of xPIC target."), &cmdlist);
