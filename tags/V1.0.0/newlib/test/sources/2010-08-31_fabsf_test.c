...
{
  int n, d;
  const unsigned N = sizeof ( fabsf_vec ) / sizeof ( fabsf_vec[0] );
  unsigned i;
  ...
  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = fabsf_vec[i][6];
    param.l_value.low = fabsf_vec[i][7];
    result.l_value.high = fabsf_vec[i][4];
    result.l_value.low = fabsf_vec[i][5];
    ...
    if (fabsf_vec[i][1] == 13)
    {
      if ( ( f_ret <= f_result+(fabsf_vec[i][2]*EPSILON) && f_ret >= f_result-(fabsf_vec[i][2]*EPSILON) ) )
      {
        return ( i );
      }
    }
    else if (fabsf_vec[i][1] == 0)
    {
      if ( !( f_ret <= f_result+(fabsf_vec[i][2]*EPSILON) && f_ret >= f_result-(fabsf_vec[i][2]*EPSILON) ) )
      {
        return ( i );
