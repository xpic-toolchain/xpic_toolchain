...
{
  int n, d;
  const unsigned N = sizeof ( expf_vec ) / sizeof ( expf_vec[0] );
  unsigned i;
  ...
  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = expf_vec[i][6];
    param.l_value.low = expf_vec[i][7];
    result.l_value.high = expf_vec[i][4];
    result.l_value.low = expf_vec[i][5];
    ...
    if (expf_vec[i][1] == 13)
    {
      if ( ( f_ret <= f_result+(expf_vec[i][2]*EPSILON) && f_ret >= f_result-(expf_vec[i][2]*EPSILON) ) )
      {
        return ( i );
      }
    }
    else if (expf_vec[i][1] == 0)
    {
      if ( !( f_ret <= f_result+(expf_vec[i][2]*EPSILON) && f_ret >= f_result-(expf_vec[i][2]*EPSILON) ) )
      {
        return ( i );
