#include "NX51_Gpio.h" // function prototypes and defines

static NX51_GPIO_AREA_T* s_ptGpio = (NX51_GPIO_AREA_T*) Addr_NX51_gpio_motion;
...
void NX51_Gpio_Sleep( unsigned long ulCounter, unsigned int uiTimeout )
{
  ...
  uiTimeout = uiTimeout * (NX51_DEV_FREQUENCY/1000000);
  ...
  s_ptGpio->aulGpio_counter_ctrl[ulCounter] |= (MSK_NX51_gpio_counter0_ctrl_run | MSK_NX51_gpio_counter0_ctrl_once  ); /* Enable the timer to one shot */

  /* poll timer ctrl for 'run' bit */
  do {
    uiVal  = s_ptGpio->aulGpio_counter_ctrl[ulCounter];
    uiVal &= MSK_NX51_gpio_counter0_ctrl_run;
  } while ( uiVal!=0 );
}
...
*                                                                            */
/*****************************************************************************/
void NX51_Gpio_CounterSetup( unsigned int uiGpioNum, unsigned long ulCounterNum, unsigned int uiMode,  unsigned int fIRQ_enable, unsigned int run_once, unsigned int fSelEvent, unsigned int ulEvent)
{
	NX51_GPIO_COUNTER0_CTRL_T Cntr;
	Cntr.bf.run       = 1;
	Cntr.bf.reserved1 = 0;
