...
///   R E T U R N
SDValue xpicTargetLowering::LowerReturn(SDValue Chain,CallingConv::ID CallConv, bool isVarArg,const SmallVectorImpl<ISD::OutputArg> &Outs,DebugLoc dl, SelectionDAG &DAG) 
{
...
printf("xpecTargetLowering::LowerReturn\n");
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getTarget(),RVLocs, *DAG.getContext());
  ...
  for (unsigned i = 0; i != RVLocs.size(); ++i)
  {
    ...
    Chain = DAG.getCopyToReg(Chain, dl, VA.getLocReg(), Outs[i].Val, Flag);
...
///   C A L L
SDValue xpicTargetLowering::LowerCall(SDValue Chain, SDValue Callee,
...
// f32 f64 i64 deleted
    ...
    if (ValToStore.getNode()) 
    {
      ...
      else
      {
        ...
        ArgFrameIdx = xRI->xpicLowerCallInfo.iCallFI;
        // its size
      ...
      MemOpChains.push_back(DAG.getStore(Chain, dl,ValToStore, PtrOff, NULL, 0,false, false, 0));
  ...
  SmallVector<CCValAssign, 16> RVLocs;
  CCState RVInfo(CallConv, isVarArg, DAG.getTarget(), RVLocs, *DAG.getContext());
  ...
  Chain = DAG.getNode(XPICISD::CALL,dl, NodeTys, Ops, InFlag.getNode() ? 3 : 2);
...
printf("xpicTargetLowering::LowerFormalArguments\n");
    ...
    switch (ObjectVT.getSimpleVT().SimpleTy)
    {
      ...
      case MVT::i32:
        ...
        {// arguments lives in stack
          int FrameIdx = MF.getFrameInfo()->CreateFixedObject(4, FrameLoadOffset,0,0);
          ...
          if (ObjectVT == MVT::i32) 
          {
            Load = DAG.getLoad(MVT::i32,dl, Chain, FIPtr, NULL, 0,false, false, 0);
          ...
          {// I am not sure that programm will comes here
            ...
            Load = DAG.getExtLoad(LoadOp,dl, MVT::i32, Chain, FIPtr,NULL, 0, ObjectVT,false, false, 0);
  ...
  if(isVarArg )
    VarArgsFrameOffset = MF.getFrameInfo()->CreateFixedObject(4, FrameLoadOffset,0,0);
...
static SDValue LowerVASTART(SDValue Op, SelectionDAG &DAG, xpicTargetLowering &TLI)
{
#ifdef DEBUG_SHOW_FNS_NAMES  
SDValueInfo("LowerVASTART",Op,&DAG); 
  ...
  SDValue FIPtr = DAG.getFrameIndex(TLI.getVarArgsFrameOffset(), MVT::i32);
...
xpicTargetLowering::xpicTargetLowering(TargetMachine &TM)
  ...
  addRegisterClass(MVT::i32, XPIC::IntRegsRegisterClass);
  addRegisterClass(MVT::i32, XPIC::WRegRegisterClass);
...
printf("LowerBR_CC()\n");
  ...
  if (LHS.getValueType() == MVT::i32) 
  {
    ...
    Opc = XPICISD::BRICC;
    CompareFlag = DAG.getNode(XPICISD::CMPICC,dl, MVT::Flag, LHS, RHS);
  } 
  else 
  {//float
    CompareFlag = DAG.getNode(XPICISD::CMPFCC,dl, MVT::Flag, LHS, RHS);
...
SDValue xpicTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) {
...
xpicTargetLowering::getRegForInlineAsmConstraint(const std::string &Constraint,
                                                  EVT VT) const {
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    case 'r':
      return std::make_pair(0U, XPIC::IntRegsRegisterClass);
    }
  }
...
std::vector<unsigned>
xpicTargetLowering::getRegClassForInlineAsmConstraint(const std::string &Constraint,
                                  EVT VT) const {
  if (Constraint.size() != 1)
  ...
  default: break;
  case 'r':
    return make_vector<unsigned>(XPIC::r2, XPIC::r3, 0);
  }
