...
asymbol *find_symbol_for_address (bfd_vma vma, struct disassemble_info *info, long *place);
			 ...
			 long *place)
{
  ...
  while (min < max
    ...
    {
      if (
//sorted_syms[min]->section == sec &&
	   info->symbol_is_valid (sorted_syms[min], info)
	  && (sorted_syms[min]->section->flags & XPIC_SEC_FLAG)!=0 )
	{
	  thisplace = min;
...
objdump_print_addr (bfd_vma vma,
		    struct disassemble_info *info,
		    bfd_boolean skip_zeroes,int twice)
{
  struct objdump_disasm_info *aux;
  asymbol *sym = NULL;
  static asymbol *old_sym=(asymbol *)1;
  /* static bfd_vma old_vma=1; */ //init with 1, but it can't be not 32 bit alligned!
...
/// xpic
// don't print the same symbol on same address twice
if(twice==1 )
{ 
 if( sym == old_sym)
{ old_sym = NULL; 
 ...
 info->fprintf_func (info->stream, _("  or  0x"));
}
else 
 info->fprintf_func (info->stream, _("// 0x"));
 old_sym = sym;

/*
// don't print the same symbol on same address twice
  if(sym == old_sym && old_vma==vma)
  {
    old_sym = NULL;
    old_vma=1;
    return;
  ...
  if(old_vma!=vma)
    {info->fprintf_func (info->stream, _("// 0x"));  old_vma=vma;}
  else
    {info->fprintf_func (info->stream, _("  or  0x")); old_vma=1;}
*/
//info->fprintf_func (info->stream, _("// 0x"));
/// end xpic
...
/* Print VMA to INFO.  This function is passed to the disassembler
   routine.  */
static void
objdump_print_address (bfd_vma vma, struct disassemble_info *info, int twice)
{
  objdump_print_addr (vma, info, ! prefix_addresses, twice);
}
	  ...
	  if (! prefix_addresses)
		...
		*--s = '0';
	  ...
	  else
	    {
	      ...
	      objdump_print_address (section->vma + addr_offset, info,0);
