...
{
  int n, d;
  const unsigned N = sizeof ( log10f_vec ) / sizeof ( log10f_vec[0] );
  unsigned i;
  ...
  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = log10f_vec[i][6];
    param.l_value.low = log10f_vec[i][7];
    result.l_value.high = log10f_vec[i][4];
    result.l_value.low = log10f_vec[i][5];

    f_param = (float) param.d_value;
    f_result = (float) result.d_value;

    f_ret = log10f(f_param);
    if (log10f_vec[i][1] == 13)
    {
      if ( ( f_ret <= f_result+(log10f_vec[i][2]*EPSILON) && f_ret >= f_result-(log10f_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (log10f_vec[i][1] == 0)
    {
      if ( !( f_ret <= f_result+(log10f_vec[i][2]*EPSILON) && f_ret >= f_result-(log10f_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
