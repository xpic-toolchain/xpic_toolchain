...
{
  int n, d;
  const unsigned N = sizeof ( cos_vec ) / sizeof ( cos_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = cos_vec[i][6];
    param.l_value.low = cos_vec[i][7];
    ret.d_value = cos(param.d_value);
    result.l_value.high = cos_vec[i][4];
    result.l_value.low = cos_vec[i][5];
    if (cos_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(cos_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(cos_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (cos_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(cos_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(cos_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
