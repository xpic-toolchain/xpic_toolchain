...
{
  int n, d;
  const unsigned N = sizeof ( tanh_vec ) / sizeof ( tanh_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = tanh_vec[i][6];
    param.l_value.low = tanh_vec[i][7];
    ret.d_value = tanh(param.d_value);
    result.l_value.high = tanh_vec[i][4];
    result.l_value.low = tanh_vec[i][5];
    if (tanh_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(tanh_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(tanh_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (tanh_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(tanh_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(tanh_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
