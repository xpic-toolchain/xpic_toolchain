...
{
  int n, d;
  const unsigned N = sizeof ( acos_vec ) / sizeof ( acos_vec[0] );
  unsigned i;
  ptr_i = &i;
  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = acos_vec[i][6];
    param.l_value.low = acos_vec[i][7];
    ret.d_value = acos(param.d_value);
    result.l_value.high = acos_vec[i][4];
    result.l_value.low = acos_vec[i][5];
    if (acos_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(acos_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(acos_vec[i][2]*EPSILON) ) )
      {
        return ( i );
      }
    }
    else if (acos_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(acos_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(acos_vec[i][2]*EPSILON) ) )
      {
        return ( i );
