...
{
  int n, d;
  const unsigned N = sizeof ( atanh_vec ) / sizeof ( atanh_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = atanh_vec[i][6];
    param.l_value.low = atanh_vec[i][7];
    ret.d_value = atanh(param.d_value);
    result.l_value.high = atanh_vec[i][4];
    result.l_value.low = atanh_vec[i][5];
    if (atanh_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(atanh_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(atanh_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (atanh_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(atanh_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(atanh_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
