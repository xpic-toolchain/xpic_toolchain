...
{
  int n, d;
  const unsigned N = sizeof ( y0_vec ) / sizeof ( y0_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = y0_vec[i][6];
    param.l_value.low = y0_vec[i][7];
    ret.d_value = y0(param.d_value);
    result.l_value.high = y0_vec[i][4];
    result.l_value.low = y0_vec[i][5];
    if (y0_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(y0_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(y0_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (y0_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(y0_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(y0_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
