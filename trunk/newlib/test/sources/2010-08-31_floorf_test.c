...
{
  int n, d;
  const unsigned N = sizeof ( floorf_vec ) / sizeof ( floorf_vec[0] );
  unsigned i;
  ...
  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = floorf_vec[i][6];
    param.l_value.low = floorf_vec[i][7];
    result.l_value.high = floorf_vec[i][4];
    result.l_value.low = floorf_vec[i][5];

    f_param = (float) param.d_value;
    f_result = (float) result.d_value;

    f_ret = floorf(f_param);
    if (floorf_vec[i][1] == 13)
    {
      if ( ( f_ret <= f_result+(floorf_vec[i][2]*EPSILON) && f_ret >= f_result-(floorf_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (floorf_vec[i][1] == 0)
    {
      if ( !( f_ret <= f_result+(floorf_vec[i][2]*EPSILON) && f_ret >= f_result-(floorf_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
