...
/* FIXME: All results in the gamma_vec table (taken from the libm test suite) are wrong.
 * This Tests only checks if the calculated results are not wrong too */
...
{
  int n, d;
  const unsigned N = sizeof ( gamma_vec ) / sizeof ( gamma_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = gamma_vec[i][6];
    param.l_value.low = gamma_vec[i][7];
    ret.d_value = gamma(param.d_value);
    result.l_value.high = gamma_vec[i][4];
    result.l_value.low = gamma_vec[i][5];
    if (gamma_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(gamma_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(gamma_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (gamma_vec[i][1] == 0)
    {
      if ( ( ret.d_value <= result.d_value+(gamma_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(gamma_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
