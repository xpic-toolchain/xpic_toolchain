...
{
  int n, d;
  const unsigned N = sizeof ( asinh_vec ) / sizeof ( asinh_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = asinh_vec[i][6];
    param.l_value.low = asinh_vec[i][7];
    ret.d_value = asinh(param.d_value);
    result.l_value.high = asinh_vec[i][4];
    result.l_value.low = asinh_vec[i][5];
    if (asinh_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(asinh_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(asinh_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (asinh_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(asinh_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(asinh_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
