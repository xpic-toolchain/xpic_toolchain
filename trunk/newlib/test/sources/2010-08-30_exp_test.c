...
{
  int n, d;
  const unsigned N = sizeof ( exp_vec ) / sizeof ( exp_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = exp_vec[i][6];
    param.l_value.low = exp_vec[i][7];
    ret.d_value = exp(param.d_value);
    result.l_value.high = exp_vec[i][4];
    result.l_value.low = exp_vec[i][5];
    if (exp_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(exp_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(exp_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
