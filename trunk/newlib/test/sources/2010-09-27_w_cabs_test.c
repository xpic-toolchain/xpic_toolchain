...
{
  int n, d;
  const unsigned N = sizeof ( hypot_vec ) / sizeof ( hypot_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param1.l_value.high = hypot_vec[i][6];
    param1.l_value.low = hypot_vec[i][7];
    c_param.x = param1.d_value;

    param2.l_value.high = hypot_vec[i][8];
    param2.l_value.low = hypot_vec[i][9];
    c_param.y = param2.d_value;

    //ret.d_value = hypot(param1.d_value,param2.d_value);
    double c_ret = cabs(c_param);

    result.l_value.high = hypot_vec[i][4];
    result.l_value.low = hypot_vec[i][5];
    double c_result = result.d_value;

    if (hypot_vec[i][1] == 13)
    {
      //if ( ( ret.d_value <= result.d_value+(hypot_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(hypot_vec[i][2]*EPSILON) ) )
      if ( ( c_ret <= c_result+(hypot_vec[i][2]*EPSILON) && c_ret >= c_result-(hypot_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (hypot_vec[i][1] == 0)
    {
      //if ( !( ret.d_value <= result.d_value+(hypot_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(hypot_vec[i][2]*EPSILON) ) )
      if ( !( c_ret <= c_result+(hypot_vec[i][2]*EPSILON) && c_ret >= c_result-(hypot_vec[i][2]*EPSILON) ) )
      {
