...
{
  int n, d;
  const unsigned N = sizeof ( y1f_vec ) / sizeof ( y1f_vec[0] );
  unsigned i;
  ...
  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = y1f_vec[i][6];
    param.l_value.low = y1f_vec[i][7];
    result.l_value.high = y1f_vec[i][4];
    result.l_value.low = y1f_vec[i][5];
    ...
    if (y1f_vec[i][1] == 13)
    {
      if ( ( f_ret <= f_result+(y1f_vec[i][2]*EPSILON) && f_ret >= f_result-(y1f_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (y1f_vec[i][1] == 0)
    {
      if ( !( f_ret <= f_result+(y1f_vec[i][2]*EPSILON) && f_ret >= f_result-(y1f_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
