...
{
  int n, d;
  const unsigned N = sizeof ( atan2_vec ) / sizeof ( atan2_vec[0] );
  unsigned i;

  for ( i = 0; i < N; ++i )
  {
    param1.l_value.high = atan2_vec[i][6];
    param1.l_value.low = atan2_vec[i][7];
    param2.l_value.high = atan2_vec[i][8];
    param2.l_value.low = atan2_vec[i][9];
    ret.d_value = atan2(param1.d_value,param2.d_value);
    result.l_value.high = atan2_vec[i][4];
    result.l_value.low = atan2_vec[i][5];
    if (atan2_vec[i][1] == 13)
    {
      if ( ( ret.d_value <= result.d_value+(atan2_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(atan2_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (atan2_vec[i][1] == 0)
    {
      if ( !( ret.d_value <= result.d_value+(atan2_vec[i][2]*EPSILON) && ret.d_value >= result.d_value-(atan2_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
