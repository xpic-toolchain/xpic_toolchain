...
/// BI: This should be replaced with constant list according to ABI 
const unsigned* xpicRegisterInfo::getCalleeSavedRegs(const MachineFunction *MF) const {
/** I don't use this function !!!!!!!
    Because additional NOP in prologue will be emitted and frame must be always created.
    See next construction(example: save r1,r2):
                       // start prologue
add r7, #-8, r7        // allocates space in stack to save register
nop                    // NOP (data dependence in r7!)
store [r7 + #0], r1    // save r1
store [r7 + #4], r2    // save r2, end prologue
                       // end prologue
....function's..body...
                       // start epilogue
load r2, [r7 + #4]     // restore r2
load r1, [r7 + #0]     // restore r1
nop                    // NOP (data dependence in r7!)
add r7, #8, r7         // delete frame
                       // end epilogue

// otherwise, I build without NOP:
                       // start prologue
store [z0 + --r7], r1  // save r1
store [z0 + --r7], r2  // save r2
add r7, #-X, r7        // IF NECESSARY, allocates space (frame) in stack to save variables and register at work
                       // end prologue
....function's..body...
                       // start epilogue
add r7, #8, r7         // IF (frame) WAS allocated in prologue, free
nop                    // NOP (data dependence in r7!). This NOP is absent if (frame) was allocated
load r2, [z0 + r7++]   // restore r2
load r1, [z0 + r7++]   // restore r1
                       // end epilogue
conclusion:
minimal benefit: 1 instruction  (NOP in prologue)
maximal benefit: 4 instructions (NOP + frame-allocation in prologue and frame-elimination + nop in epilogue)
*/
  static const unsigned CalleeSavedRegs[] = { 0 };
  return CalleeSavedRegs;
}

const TargetRegisterClass* const* xpicRegisterInfo::getCalleeSavedRegClasses(const MachineFunction *MF) const {
...
unsigned xpicRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II, int SPAdj, int *Value, RegScavenger *RS) const {
 #ifdef DEBUG_SHOW_FNS_NAMES  
 printf("eliminateFrameIndex\n");
  ...
  { /*construct sequence:
    ...
    */
    ...
    // get GlobalVariable
    GlobalVariable *NewGV = xpicHlprGetGlobalVariable(M, val);
    // debug location:
    DebugLoc dl = (II != (MF.begin())->end() ? II->getDebugLoc() : DebugLoc::getUnknownLoc());
...
printf("xpicRegisterInfo::emitEpilogue\n");
  ...
  --MBBI;
  // debug location:
  DebugLoc dl = (MBBI != MBB.end() ? MBBI->getDebugLoc() : DebugLoc::getUnknownLoc());
...
std::vector <GlobalVariable *> vecGVtoKill;
 ...
 {// annotation table is not constructed, make it
  ...
  for( Module::const_global_iterator IB_GVL=M->global_begin(), IE_GVL=M->global_end();IB_GVL!=IE_GVL;++IB_GVL){
   // current Global Variable
   const GlobalVariable *GVar=IB_GVL;
   // has it name?
   ...
   vecGVtoKill.push_back((GlobalVariable *)GVar);
...
//printf("FunctionName: %s ",FA.FunctionName.data());
         ...
         const Constant *ConstFieldSubPointer  = dyn_cast<Constant>(ConstStrField->getOperand(0));
...
bool xpicRegisterInfo::IsDeleteAllowed(const Value *V)const {
  Value::use_const_iterator I = V->use_begin(), E = V->use_end();
  if (I == E) return true;
