...
{
  int n, d;
  const unsigned N = sizeof ( sinf_vec ) / sizeof ( sinf_vec[0] );
  unsigned i;
  ...
  for ( i = 0; i < N; ++i )
  {
    param.l_value.high = sinf_vec[i][6];
    param.l_value.low = sinf_vec[i][7];
    result.l_value.high = sinf_vec[i][4];
    result.l_value.low = sinf_vec[i][5];
    ...
    if (sinf_vec[i][1] == 13)
    {
      if ( ( f_ret <= f_result+(sinf_vec[i][2]*EPSILON) && f_ret >= f_result-(sinf_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
      }
    }
    else if (sinf_vec[i][1] == 0)
    {
      if ( !( f_ret <= f_result+(sinf_vec[i][2]*EPSILON) && f_ret >= f_result-(sinf_vec[i][2]*EPSILON) ) )
      {
        return ( -1 );
