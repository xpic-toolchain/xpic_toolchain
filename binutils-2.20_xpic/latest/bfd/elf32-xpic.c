/* XPIC-specific support for 32-bit ELF
...
#define SHOW_RELOC_OVERFLOW

/* Description of constant definition (for more information see "./bfd/reloc.c"):

R_XPIC_24_PCREL (supported complex relocation handling)
  Jump commands:  jmp R_XPIC_24_PCREL;                             24bit,  pc relative, const = pc + -2^25 � 2^25, (dword aligned)

R_XPIC_Y17_PCREL (supported complex relocation handling)
  Load command:  load reg, s[base_reg + R_XPIC_Y17_PCREL]b;        17bit,  pc relative, const = -2^16 / 17 / 18 � 2^16 / 17/ 18 (dword, word or byte aligned)

R_XPIC_Z19 (supported complex relocation handling)
  Load command:  load reg, R_XPIC_Z19;                             19bit,  const = -2^18 � 2^18

--------------------

R_XPIC_X15_PCREL
  ALU command:  opcode reg, [pc + R_XPIC_X15_PCREL], reg;          15bit,  pc relative, const = -2^16 � 21^6 (dword aligned)

R_XPIC_Y17
  Load command:  load reg, s[z0 + R_XPIC_Y17]b;                    17bit,  const = -2^16 / 17 / 18 � 2^16 / 17/ 18 (dword, word or byte aligned)


WARNING:
With complex relocation handling is the next situation dangerous:

load pc, ......
_some_instruction_with_complex_relocation_handling (it will add additional instructions, but after PC targeting executes only one instruction!)
...
static reloc_howto_type elf_xpic_howto_table[] =
{
  ...
  HOWTO (R_XPIC_X15_PCREL,       /*  type < ALU_opcode rx, [pc + #const], rx >   */
	 0,	                         /* rightshift                                   */
	 2,                          /* size (0 = byte, 1 = short, 2 = long)         */
	 15,                         /* bitsize                                      */
	 TRUE,                       /* pc_relative                                  */
  ...
  /* A 15 bit relocation, dependend from memory access  (byte, word, dword aligned)   */
  HOWTO (R_XPIC_X15,             /* type < ALU_opcode rx, [#const], rx >              */
	 0,                          /* rightshift                                        */
	 2,                          /* size (0 = byte, 1 = short, 2 = long)              */
	 15,                         /* bitsize                                           */
	 FALSE,                      /* pc_relative                                       */
...
char *XPIC_get_internal_relocation_symbolname(bfd *abfd,Elf_Internal_Rela *irel);
bfd_boolean xpic_get_symbol_value2(bfd *abfd,Elf_Internal_Rela *irel,bfd_vma *symval);
...
/// saturation for X constant signed
#define xpic_saturateX(v)   ( (bfd_signed_vma)v <   -65536 || (bfd_signed_vma)v >   65535 )
/// saturation for X constant Unsigned
#define xpic_saturateXU(v)   ( (bfd_vma)v > 131071 )
/// saturation for Jump
...
printf("\n ... R_XPIC_24_PCREL = %d (0x%x)\n",(int)srel,(unsigned int)srel);
#endif
      if(xpic_saturateJMP(srel))
      {
#ifdef SHOW_RELOC_OVERFLOW
        printf("... OUT OF RANGE: R_XPIC_24_PCREL %d\n",(int)srel);
#endif
        return bfd_reloc_outofrange;
      }
...
printf("\n ... R_XPIC_Y17_PCREL = %d (0x%x)\n",(int)srel,(unsigned int)srel);
      ...
      {
#ifdef SHOW_RELOC_OVERFLOW
        printf("... OUT OF RANGE: R_XPIC_Y17_PCREL %d\n",(int)srel);
#endif
        return bfd_reloc_outofrange;
      }
      srel&=0x0001ffff;					// 17 bit mask
...
printf("\n ... R_XPIC_X15_PCREL = %d (0x%x)\n",(int)srel,(unsigned int)srel);
      ...
      {
#ifdef SHOW_RELOC_OVERFLOW
        printf("... OUT OF RANGE: R_XPIC_X15_PCREL %d\n",(int)srel);
#endif
        return bfd_reloc_outofrange;
      }
      srel>>=2;
...
printf("\n ... R_XPIC_X15 = %d (0x%x)\n",(int)srel,(unsigned int)srel);
#endif
      if( xpic_saturateXU(srel) )
      {
#ifdef SHOW_RELOC_OVERFLOW
        printf("... OUT OF RANGE: R_XPIC_X15 0x%08x\n",(unsigned int)srel);
#endif
        return bfd_reloc_outofrange;
      }
      srel>>=2;
...
// This function returns the name of symbol, that is linked with Elf_Internal_Rela
...
char *XPIC_get_internal_relocation_symbolname(bfd *abfd,Elf_Internal_Rela *irel)
{
  ...
  if(symtab_hdr==NULL || isymbuf==NULL)
  {
    printf("XPIC_get_internal_relocation_symbolname: pointer symtab_hdr or isymbuf is NULL\n");
    return NULL;
  }
  ...
  if (ELF32_R_SYM (irel->r_info) < symtab_hdr->sh_info)
  {
    ...
    string_hdr = elf_elfsections (abfd)[shindex];

    if(isym->st_name==0)
      return NULL;

    if(string_hdr->sh_size > isym->st_name)
    {
      char *str = (char *)string_hdr->contents;
      if(str!=0)
      {
        return str+isym->st_name;
      }
    }
  }
  return NULL;
  ...
  bfd_vma relocation;
      ...
      if (ELF32_R_SYM (irel->r_info) < symtab_hdr->sh_info)
      {
        ...
        *symval = isym->st_value;
        ...
        if ((sec->flags & SEC_MERGE)&& ELF_ST_TYPE (isym->st_info) == STT_SECTION&& sec->sec_info_type == ELF_INFO_TYPE_MERGE)
        {
          irel->r_addend =_bfd_merged_section_offset (abfd, psec,elf_section_data (sec)->sec_info, isym->st_value + irel->r_addend);
          sec=*psec;
          irel->r_addend -= relocation;
          irel->r_addend += sec->output_section->vma + sec->output_offset;
        }
      }
      else
      {
                         ...
                         bfd_boolean *again)
{
  ...
  Elf_Internal_Rela *irel, *irelend;
  Elf_Internal_Rela  p_irelCopy;
  Elf_Internal_Rela  *irelCopy=&p_irelCopy;
...
//      || (sec->flags & SEC_LOAD) == 0
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	...
	// Make copy of reloc:
        memcpy(irelCopy ,irel,sizeof(Elf_Internal_Rela));
        // Function 'xpic_get_symbol_value2' can overwrite structure "irelCopy" (in case of sections with SEC_MERGE flag), in field "->r_addend". This field is used just to determinate a target location, in all other calculations use "irel". Don't apply for jump.
     // Get the value of the symbol referred to by the reloc.
      if(!xpic_get_symbol_value2(abfd,irelCopy,&symval)) continue;
...
==Far load instruction: load rx, s[pc + #offset]b. Here pc is address of current instruction.
  ...
  3: Loadable data not accessible with standard load. Construct Load via .data section:
      .section .text
...
==Far store instruction: store s[pc + #offset]b, rx
  ...
  3: Addres to store not accessible with standard store. Construct Store via .data section:
      .section .text
...
stored_address_to_store:
      $address_to_store
**/
			unsigned int code_instr;
			unsigned int code_new_instr;
			unsigned int mas;

	    bfd_signed_vma address_to_load = symval + irelCopy->r_addend;
	    bfd_signed_vma distance;
	    // Get the address of current instruction:
	    bfd_vma address_of_this_instr;
	    address_of_this_instr = (sec->output_section->vma + sec->output_offset + irel->r_offset);
	    ...
	    if(align){
	      (*link_info->callbacks->einfo)(_("%P%F: misaligned memory access.\n"));
	      goto error_return;// error, misalignment
	    }
...
// FIXME: BI: Y17 relax does not work when target symbol is not yet located
	    ...
	    unsigned int opcode = code_instr&0xe0000000;
	    /// I. replace CURRENT instruction 'load reg, s[pc + #off]b' with 'load r?, [pc + #off_stored_addr_to_load]d'
...
add_nop_load:
	    ...
	    // set register to new instruction and add this instruction the section:
	    code_new_instr |= reg;
	  ...
	  break;
         ...
         case R_XPIC_Z19:
          {
/**
To analyze: Far load instruction: load rx, #const.
...
// FIXME: BI: 
	 ...
	 break;
         ...
         case R_XPIC_X15_PCREL:
          {
/** THIS  RELOCATION  INCLUDES  'ALU' instructions (add, or, and, ... )!!!
To analyze:
==   ALU instruction: "alu_opcode rx_dst, [pc + #offset], rx_src2". Here pc is address of current instruction.
  1: If #offset not more than X constant (15 signed bits shifted twice right), do nothing, return.

  2: Else: offset data can be inside INT_DRAM (starts from 0), and if (#offset+pc) not greater
     than XU constant (15 unsigned bits shifted twice right [0 - 128K]), use "alu_opcode rx, [(#offset+pc)], rx"

  3: The data (#offset+pc) is not accessible neither with "alu_opcode rx, [pc + #offset], rx" nor "alu_opcode rx, [(#offset+pc)], rx".
     Construct some additional loadLoad via .data section:
      .section .text
      load r6, [pc + #offset_to stored_address_to_load]d
      nop
      alu_opcode rx_dst, [r6], rx_src2
   ........................................................
     .section .data
stored_address_to_load:
      (#offset+pc)
**/
	    ...
	    /// 1. Is this offset for standard alu_opcode valid?
	    if( xpic_saturateX(distance)==0 )
	      break;// yes, use standard load

	    /// 2. Is loadable data inside INT_DRAM?
	    if( xpic_saturateXU(address_to_load)==0 )
        {// YES, offset inside INT_DRAM (0-128K)!
	      // replace with new instruction ("alu_opcode rx, [pc + #offset], rx"   ---->   "alu_opcode rx, [(#offset+pc)], rx")
	      code_new_instr = (code_instr & 0xfffe7fff)|(0x00008000);
	      bfd_put_32(abfd, code_new_instr , contents + irel->r_offset);
	      // Fix the relocation's type
	      irel->r_info = ELF32_R_INFO (ELF32_R_SYM (irel->r_info),R_XPIC_X15);
	      break;
	    }

	    /// 3. Loadable data is to far for this alu_opcode, construct FarLoad via .data section
	    // construct sequence for load:
	    //  load r?, [pc + #offset_to stored_address_to_load]
	    //  nop
	    //  alu_opcode rx, [z0 + r?]d, rx

	    // In first part (I.) correct instruction and relocation, second (II.) - nop (data dependence) and third (III.) - load data from pointer/store data to pointer

        /// I.a replace CURRENT instruction 'alu_opcode reg_src, [pc + #off], reg_dst' with 'load r?, [pc + #off_stored_addr_to_load]d'
	    // set: mas=b10(4byte), sign=0 (unsigned)
	    code_new_instr = XPIC_INS_LOAD_RX_MPC_OFF | (XPIC_FARSTORE_REG<<25);
	    // update CURRENT instruction:
	    bfd_put_32(abfd, code_new_instr , contents + irel->r_offset);


	    // I.b work for relocation: in data section must be placed address to load, and link (relocation) to it
	    asection *data=NULL;
	    bfd_vma  addr_of_stored_addr_to_load;
	    // Get '.data' section
            data= xpic_get_data_section_cached(abfd,link_info);
...
skip_symbol_creation:
	    ...
	    // III. Insert after next, instruction with indirect address access from pointer r?: alu_opcode rx_dst, [r?]d, rx_src2

	    // In 'code_instr' are saved destination register 'reg', mas, sign. Use it next, but replace 'base_reg' (mask: 0xfe1fffff) with current 'reg' and set offset 'const' null (mask: 0xfffe0000):
	    code_new_instr = code_instr &( 0xfe1fffff & 0xfffe0000 ); // set masks
	    // it was
	    int reg_srs2 = code_instr & 0x001e0000;		// source2 register, shifted left on 17
	    int reg_dst  = code_instr & 0x01e00000;		    // destination register, shifted left on 21

	    // construct new instruction with old dst and src2 registers and add this instruction te section:
	    //               alu_opcode                       rx_dst, [r?]d, rx_src2
	    code_new_instr |= XPIC_INS_RDST_ALU_MSRC1__RSRC2 | reg_dst | XPIC_FARSTORE_REG | reg_srs2;
